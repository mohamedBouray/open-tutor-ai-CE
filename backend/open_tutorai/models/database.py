...
class Classe(Base):
    ...
    courses = relationship("CourseContent", back_populates="classe", cascade="all, delete-orphan")
...
class StudentActivity(Base):
    ...
    user_id = Column(String, ForeignKey(f"{PREFIX}user.id")) # Awla 'users.id' 3la 7ssab smit table dyalk
    classe_id = Column(String, ForeignKey(f"{PREFIX}classe.id", ondelete="CASCADE"), nullable=True)
...
class CourseFile(Base):
    ...
    id = Column(String, primary_key=True, index=True)
    course_id = Column(String,ForeignKey(f"{PREFIX}course_content.id", ondelete="CASCADE"),nullable=False)
    filename = Column(String, nullable=False)
    file_path = Column(String, nullable=False)
    file_type = Column(String, nullable=True)
    file_size = Column(Integer, nullable=True)
