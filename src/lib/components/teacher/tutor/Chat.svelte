...
<script lang="ts">
	...
	import Navbar from '$lib/components/teacher/tutor/ChatNavbar.svelte';
	...
	const initNewChat = async () => {
		...
		if ($page.url.pathname.includes('/c/')) {
			window.history.replaceState(history.state, '', `/teacher/Classes/c/`);
		}
	...
	const initChatHandler = async (history) => {
		...
		try {
			...
			if (!$temporaryChatEnabled) {
				...
				window.history.replaceState(history.state, '', `/teacher/Classes/c/${_chatId}`);
