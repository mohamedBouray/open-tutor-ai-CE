<script lang="ts">
    ...
    async function AddAssignment() {
        if (!assignmentTitle.trim() || !assignmentDescription.trim()  || !assignmentDueDate.trim() || assignmentMaxPoints <= 0 || !assignmentClass) {
...
<main class="fixed inset-0 z-[1000] flex items-center justify-center bg-slate-900/70 dark:bg-black/80 p-5 backdrop-blur-sm transition-all">
    <div class="flex max-h-[90vh] w-full max-w-[520px] origin-center animate-pop flex-col overflow-hidden rounded-[24px] bg-white dark:bg-gray-900 p-8 shadow-2xl transition-colors duration-200">
        <div class="flex flex-shrink-0 items-center justify-between border-b border-gray-200 dark:border-gray-800 pb-4">
            ...
            <button class="flex h-8 w-8 items-center justify-center rounded-lg bg-transparent text-gray-500 transition-all hover:rotate-90 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white"
        ...
        <div class="flex-grow overflow-y-auto py-6 font-['Inter']">
            ...
            <div class="mb-5 flex flex-col gap-2">
                ...
                <textarea id="desc" rows="4" bind:value={assignmentDescription}
                    class="rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 p-3 text-[15px] text-gray-900 dark:text-white outline-none transition-all focus:border-blue-500 focus:ring-3 focus:ring-blue-500/10"
            ...
            <div class="mt-4 flex gap-3">
                <button class="flex-1 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 py-3 text-[15px] font-semibold text-gray-700 dark:text-gray-300 transition-all hover:bg-gray-50 dark:hover:bg-gray-700"
                    on:click={closeModal}>
                    {$i18n.t('Cancel')}
                </button>
                <button class="flex-1 rounded-lg border-none bg-blue-600 py-3 text-[15px] font-semibold text-white transition-all hover:bg-blue-700 active:scale-[0.98]"
